...
| status    | String            | required, enum: 'pending', 'accepted', 'complete', 'rejected' | Game's status: if either swiped "no", it's rejected. If one has swiped "yes", and the other has done nothing, it's "pending". If it's already been played, "completed". And if it's been accepted by both users, but has not yet been played, it's "accepted". |
